...
class QuantityOCR:
    ...
    def _fraction_variants(self, roi: np.ndarray) -> list[np.ndarray]:
        ...
        up_color = cv2.resize(color, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
        up_gray = cv2.resize(gray, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
        _, otsu = cv2.threshold(up_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        return [up_color, otsu, cv2.bitwise_not(otsu)]
